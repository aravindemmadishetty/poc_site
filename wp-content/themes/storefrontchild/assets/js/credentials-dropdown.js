(function($){
    $(document).ready(function(){
        ...
        var data = cfwc_ticket_type_dropdown_data;
        var countries = [
        ...
        function customPopulateCredentialsDropdown(ticketType){
            ...
            var html = customTableCellForFields(
                {
                    id : "ticket-type-credentials",
                    options : credentials.map( cred => [cred, cred]),
                    label: "Credentials"
                },
                customPopulateSelectDropdown
            );

            return html;
        }

        function customPopulateSelectDropdown(args){
            var options = `<option value>Choose an option</option>`;
        ...
        function customTableCellForFields(args, callBack){
            var html = `<td class="label"><label for="${args['id']}">${args['label']}</label></td>
            <td class="value">${callBack(args)}</td>`;
        ...
        data['attendee_data'].forEach( field => {
            if(field['type']=='dropdown'){
                if(field['id']=='attendee_country'){
                    ...
                    }, customPopulateSelectDropdown)}</tr>`);
                }
            }
            else $('.variations tbody').append(`<tr>${customTableCellForFields(field, customPopulateTextBox)}</tr>`);
        })
